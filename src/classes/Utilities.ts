...
export default abstract class Utilities {
  ...
  public static async createNode(
    ...
    const promises = controlPoints.map(async (endpoint) => {
      return this.linkEndpointToProfil(
  ...
  public static async linkEndpointToProfil(
    controlPointContextId: string,
    groupNodeId: string,
    endpoint: IControlEndpoint
  ): Promise<string> {
    ...
    await (<any>netWorkService)._createAttributes(
      endpointObj.childId,
      endpointObj.res
    );
  ...
  public static createEndpointNode(obj: IControlEndpoint): {
    childId: string;
  ...
  public static getDifference(
    oldEndpoint: Array<IControlEndpoint>,
    newEndpoints: Array<IControlEndpoint>
  ): IDiffResult {
    ...
    const toUpdate = newEndpoints.filter((el) =>
      this.isUpdated(el, oldEndpoint)
    );
  ...
  public static isUpdated(
    ...
    const configAreEquals = this.configAreEquals(
      controlPoint.config,
      found.config
    );
  ...
  public static configAreEquals(
    config1: BoolConfigDataType | EnumConfigDataType | NumberConfigDataType,
    config2: BoolConfigDataType | EnumConfigDataType | NumberConfigDataType
  ): boolean {
  ...
  public static objectsAreEquals(
  ...
  public static create(
    controlPointContextId: string,
    newList: Array<IControlEndpoint>,
    profils: SpinalNodeRef[],
    endpointsLst
  ): Promise<string[][]> {
  ...
  public static update(
    ...
    const promises = newList.map((element) => {
      ...
      const promises2 = profils.map(async (profil) => {
        const endpointId = await this.getEndpointByType(
          profil.id.get(),
          element.id
        );
  ...
  public static async modEndpoint(
    endpointId: string,
    newProfil: { name: string; [key: string]: any }
  ): Promise<void> {
  ...
  public static setProfilValue(
    newProfil: { [key: string]: any },
    oldProfil: { [key: string]: any }
  ): void {
    for (const key of Object.keys(newProfil)) {
  ...
  public static async getEndpointByType(
    profilId: string,
    endpointId: string
  ): Promise<string> {
  ...
  public static getProfilEndpoints(profilId: string): Promise<SpinalNodeRef[]> {
    return SpinalGraphService.getChildren(profilId, [
      SpinalBmsEndpoint.relationName,
    ]);
  }
  ...
  public static async linkProfilToGroupItemIfNotExist(
    itemId: string,
    controlPointContextId: string,
    controlPointId: string,
    controlPoints: { name: string; endpoints: any }
    ...
    const children = await SpinalGraphService.getChildren(itemId, [
      ROOM_TO_CONTROL_GROUP,
    ]);
    const found = children.find(
      (el) => el.referenceId?.get() === controlPointId
    );

...
export class SpinalControlEndpointService {
  ...
  public createControlPointProfil(
    contextId: string,
    groupId: string,
    controlPointProfil: { name: string; endpoints: Array<IControlEndpoint> } = {
      name: 'unknow',
      endpoints: [],
    }
  ): Promise<any> {
    const profilNodeId = SpinalGraphService.createNode(
      { name: controlPointProfil.name, type: this.CONTROL_POINT_TYPE },
  ...
  ): Promise<{ name: string; endpoints: any }> {
  ...
  private async getGroupItems(nodeId: string): Promise<Array<any>> {
    ...
    const promises = groups.map((el) =>
      groupManagerService.getElementsLinkedToGroup(el.id.get())
    );
  ...
  private async linkEndpointToProfil(
    controlPointContextId: string,
    groupNodeId: string,
    endpoint: any
  ) {
  ...
  private createEndpointNode(obj: any): {
    childId: string;
  ...
  private async controlPointProfilIsAlreadyLinked(
    profilId: string,
    groupId: string
  ): Promise<boolean> {
  ...
  private listenLinkItemToGroupEvent() {
    spinalEventEmitter.on(
      groupManagerService.constants.ELEMENT_LINKED_TO_GROUP_EVENT,
      async (data) => {
  ...
  private listenUnLinkItemToGroupEvent() {
    spinalEventEmitter.on(
      groupManagerService.constants.ELEMENT_UNLINKED_TO_GROUP_EVENT,
      async (data) => {
        const profilsLinkedModel = await this.getElementLinked(data.groupId);
        const elementProfilsModel = SpinalGraphService.getChildren(
          data.elementId,
          [this.ROOM_TO_CONTROL_GROUP]
        );

        const profilsLinked = profilsLinkedModel.map((el: any) => el.get());
        const elementProfils = (await elementProfilsModel).map((el) =>
          el.get()
        );

        const promises = elementProfils.map((profil) => {
          const found = profilsLinked.find(
            (el) => el.id === profil.referenceId
          );
          if (found) {
            return SpinalGraphService.removeChild(
              data.elementId,
